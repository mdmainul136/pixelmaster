...
const EventDebugger = ({ auth, initialLogs = [] }) => {
  ...
  useEffect(() => {
    ...
    const startPolling = () => {
      ...
      console.log("Live Debugger: Switching to Polling Mode (Artisan Serve Safe)");
      setIsPolling(true);
      const fetchEvents = async () => {
        ...
        try {
          ...
          console.error("Live Debugger: Polling failed", err);
        }
      };
      fetchEvents();
      pollInterval = setInterval(fetchEvents, 3e3);
    };
    const connect = () => {
      if (!isLive || isNavigatingRef.current) return;
      ...
      try {
        ...
        eventSource.addEventListener("close", (e) => {
          eventSource.close();
          if (!isNavigatingRef.current) connect();
        });
        ...
        eventSource.onerror = (err) => {
          eventSource.close();
          if (isLive && !isNavigatingRef.current) {
            startPolling();
          }
        };
      } catch (e) {
