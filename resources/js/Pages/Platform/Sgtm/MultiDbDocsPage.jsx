...
import {
  Database, Shield, Package, TrendingUp, BarChart3,
  CheckCircle2, AlertTriangle, ChevronDown, ChevronRight,
  Info, Lightbulb, Zap, Lock, Server, Globe,
  Users, HardDrive, ArrowRight, X,
} from "lucide-react";
...
const CompareRow = ({ label, single, multi, winner }) => (
  <tr className="border-b border-white/5 hover:bg-white/3 transition-colors">
    <td className="py-3 pr-4 text-xs font-medium text-slate-300">{label}</td>
    <td className="py-3 pr-4">
      <div className={`flex items-center gap-1.5 text-xs ${winner === "single" ? "text-emerald-400" : "text-red-400"}`}>
        {winner === "single" ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0" /> : <X className="h-3.5 w-3.5 shrink-0" />}
        <span>{single}</span>
      </div>
    </td>
    <td className="py-3">
      <div className={`flex items-center gap-1.5 text-xs ${winner === "multi" ? "text-emerald-400" : winner === "both" ? "text-emerald-400" : "text-slate-400"}`}>
        {(winner === "multi" || winner === "both") ? <CheckCircle2 className="h-3.5 w-3.5 shrink-0" /> : <X className="h-3.5 w-3.5 shrink-0" />}
        <span>{multi}</span>
      </div>
    </td>
  </tr>
...
const ReasonCard = ({ num, icon: Icon, color, iconBg, title, subtitle, simple, points, code }) => (
  <div className={`rounded-2xl border overflow-hidden ${color}`}>
    <div className="flex items-start gap-4 p-6 pb-4">
      ...
      <div>
        <div className="flex items-center gap-2 mb-0.5">
          <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">কারণ {num}</span>
        </div>
    ...
    <div className="px-6 pb-6 space-y-3">
      ...
      {code && (
        <div className="rounded-lg bg-black/30 border border-white/10 px-4 py-3 font-mono text-[10px] text-emerald-400 leading-relaxed whitespace-pre">
          {code}
        </div>
      )}
    </div>
...
export default function MultiDbDocsPage() {
  const toc = [
  ...
  return (
    <PlatformLayout>
      <Head title="Multi-DB Architecture | PixelMaster Platform" />
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-900 via-[hsl(222,47%,11%)] to-slate-900 border border-white/10 p-10 mb-8 shadow-2xl">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 rounded-full bg-purple-500/20 border border-purple-500/30 px-4 py-1.5 mb-5">
            <Database className="h-3.5 w-3.5 text-purple-400" />
            <span className="text-[11px] font-bold text-purple-400 uppercase tracking-wider">Architecture Deep Dive · Multi-Tenant DB</span>
          </div>
          <h1 className="text-4xl font-black text-white tracking-tight mb-3 leading-tight">
            কেন Tenant-wise Multi-DB<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400">
              PixelMaster-এর জন্য বাধ্যতামূলক?
            </span>
          </h1>
          <p className="text-slate-400 text-base max-w-2xl leading-relaxed mb-6">
            Stape, Vercel, Shopify-এর মতো Enterprise SaaS প্ল্যাটফর্মের স্ট্যান্ডার্ড আর্কিটেকচার।
            কেন এটা শুধু "ভালো অপশন" নয়, বরং Scale করতে হলে এর কোনো বিকল্প নেই — পাঁচটি সলিড কারণসহ।
          </p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[230px_1fr] gap-8">
        ...
        <div className="hidden xl:block">
          <div className="sticky top-8 rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm">
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-4">এই পেজে</p>
        ...
        <div className="space-y-6 min-w-0">
          ...
          {/* 1 — Why */}
          <Section id="why" icon={Lightbulb} title="কেন Multi-DB 'বাধ্যতামূলক'?"
            badge="Core Architecture" subtitle="সহজ ভাষায় বোঝা যাক"
            color="bg-gradient-to-r from-purple-600/30 to-blue-600/20">

            <Callout color="blue" title="সহজ উদাহরণ">
              ভাবুন একটি বড় অ্যাপার্টমেন্ট বিল্ডিং। <strong className="text-white">Single Database = একটাই বড় হল ঘর</strong> যেখানে সবাই থাকে।
              যদি একজন জোরে চিৎকার করে বা কোনো সমস্যা তৈরি করে, সবাই ক্ষতিগ্রস্ত।<br /><br />
              <strong className="text-white">Multi-DB = প্রতিটা ফ্ল্যাট আলাদা, লক করা।</strong> একটি ফ্ল্যাটে সমস্যা হলে পাশের ফ্ল্যাট কেউ জানেই না।
            </Callout>
          ...
          {/* 2 — Compare */}
          <Section id="compare" icon={BarChart3} title="Single DB vs Multi-DB — পূর্ণ তুলনা"
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="text-left py-3 pr-4 text-slate-500 font-bold uppercase tracking-wider">বিষয়</th>
                    <th className="text-left py-3 pr-4 text-slate-400 font-bold">❌ Single Database</th>
                    <th className="text-left py-3 text-emerald-400 font-bold">✅ Multi-DB (Tenant-wise)</th>
                  </tr>
                </thead>
                <tbody>
                  <CompareRow label="১ কোটি ইভেন্টের পরে পারফরম্যান্স" single="ভয়াবহ ধীর, টাইমআউট" multi="আগের মতোই দ্রুত" winner="multi" />
                  <CompareRow label="একজনের লোড অন্যকে প্রভাবিত করে?" single="হ্যাঁ, সবাই ক্ষতিগ্রস্ত" multi="না, পুরোপুরি বিচ্ছিন্ন" winner="multi" />
                  <CompareRow label="GDPR Physical Isolation" single="নেই — শেয়ার্ড টেবিল" multi="আছে — আলাদা DB" winner="multi" />
                  <CompareRow label="একজনের ডাটা এক্সপোর্ট করা" single="লাখ রো ফিল্টার করতে হবে" multi="একটি mysqldump কমান্ড" winner="multi" />
                  <CompareRow label="Storage Quota সঠিকভাবে মাপা" single="প্রায় অসম্ভব" multi="SELECT data_length..." winner="multi" />
                  <CompareRow label="হ্যাক হলে ক্ষতির পরিসর" single="সব ক্লায়েন্টের ডাটা ফাঁস" multi="শুধু সেই একটি DB" winner="multi" />
                  <CompareRow label="মডিউল-নির্দিষ্ট Schema" single="সবার জন্য একই টেবিল" multi="প্রয়োজন অনুযায়ী টেবিল" winner="multi" />
                  <CompareRow label="ডেভেলপমেন্ট জটিলতা" single="সহজ শুরু" multi="শুরুতে একটু বেশি" winner="single" />
                  <CompareRow label="Deadlock / Lock Contention" single="খুব বেশি ঝুঁকি" multi="শূন্য — আলাদা DB" winner="multi" />
                  <CompareRow label="একজন ক্লায়েন্ট সাসপেন্ড করা" single="রো-লেভেল ফিল্টার লাগবে" multi="DB বন্ধ করলেই হলো" winner="multi" />
                  <CompareRow label="১+ বছর পরে ব্যবহারযোগ্যতা" single="প্রায় অসম্ভব" multi="সহজেই পারফরম্যান্ট" winner="multi" />
                </tbody>
              </table>
            </div>

            <div className="mt-4">
              <Callout color="amber" icon={AlertTriangle} title="একটি গুরুত্বপূর্ণ সত্য">
                Single Database-এ শুরু করা সহজ। কিন্তু যখন Scale করার সময় আসে, তখন Multi-DB-এ মাইগ্রেট করা
                প্রায় অসম্ভব — লাখ লাখ রো রিঅর্গানাইজ করতে হবে, downtime হবে, ডাটা হারানোর ঝুঁকি থাকবে।
                <strong className="text-white"> শুরু থেকে সঠিক আর্কিটেকচার বেছে নেওয়াই বুদ্ধিমানের কাজ।</strong>
              </Callout>
            </div>
          </Section>

          {/* 5 reasons */}
          <div id="reason1" className="scroll-mt-6">
            <ReasonCard
              num="১" icon={TrendingUp}
...
-- Query Time: 1.2 seconds ← কারণ ৯০% ডাটা নেই`}
          ...
          <div id="reason2" className="scroll-mt-6">
            <ReasonCard
              num="২" icon={Shield}
...
tenancy()->end(); // → পরের ক্লায়েন্টের জন্য ready`}
          ...
          <div id="reason3" className="scroll-mt-6">
            <ReasonCard
              num="৩" icon={HardDrive}
...
# অন্য কোনো ক্লায়েন্টের কোনো প্রভাব নেই`}
          ...
          <div id="reason4" className="scroll-mt-6">
            <ReasonCard
              num="৪" icon={Zap}
...
-- এবং এই query টি ১ কোটি row-এ ১ মিনিট সময় নেয়`}
          ...
          <div id="reason5" className="scroll-mt-6">
            <ReasonCard
              num="৫" icon={Package}
...
// ফলাফল: acme-store-এর DB-তে মাত্র ৪০ টেবিল`}
          ...
          {/* How it works in PixelMaster */}
          <Section id="howworks" icon={Server} title="PixelMaster-এ কীভাবে এটা কাজ করে?"
            badge="Implementation" subtitle="Stancl Tenancy + Kafka + ClickHouse — তিন স্তরের isolation"
            ...
            <div className="grid md:grid-cols-3 gap-4 mb-5">
              ...
              ].map(({ layer, color, icon: Icon, iconColor, how, example }) => (
                <div key={layer} className={`rounded-xl border ${color} bg-white/5 p-4`}>
                  <div className="flex items-center gap-2 mb-3">
                    <Icon className={`h-4 w-4 ${iconColor}`} />
                    <p className="text-xs font-bold text-white">{layer}</p>
                  </div>
                  <p className="text-[11px] text-slate-400 leading-relaxed mb-3">{how}</p>
                  <div className="rounded-lg bg-black/30 border border-white/10 px-3 py-2 font-mono text-[10px] text-emerald-400 whitespace-pre">{example}</div>
                </div>
              ))}
            </div>

            <Callout color="green" title="Stancl Tenancy — আমাদের Multi-DB মেশিন">
              PixelMaster ব্যবহার করে <strong className="text-white">stancl/tenancy</strong> Laravel package।
              এটা automatically প্রতিটি Tenant অনুযায়ী MySQL connection switch করে।
              Kafka Consumer daemon-এ <code className="text-white mx-1">tenancy()-&gt;initialize($tenant_id)</code> একটাই লাইন —
              এরপর থেকে সব query সরাসরি ওই ক্লায়েন্টের private database-এ যায়।
              <code className="text-white mx-1">finally {'{'} tenancy()-&gt;end(); {'}'}</code>
              নিশ্চিত করে যে context কখনো পরের message-এ leak করে না।
            </Callout>
          </Section>

          {/* Real World */}
          <Section id="realworld" icon={Globe} title="Real-World প্রমাণ — বড় কোম্পানিরা কী করে?"
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              ].map(({ company, what, how, color, badge, badgeColor }) => (
                <div key={company} className={`rounded-xl border ${color} p-5`}>
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div>
                      <p className="text-sm font-bold text-white">{company}</p>
                      <p className="text-[10px] text-slate-500">{what}</p>
                    </div>
                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase ${badgeColor}`}>{badge}</span>
                  </div>
            ...
            <div className="mt-5 rounded-xl border border-indigo-500/20 bg-indigo-500/10 p-5">
              <p className="text-sm font-bold text-white mb-2">PixelMaster এই কোম্পানিগুলোর মতোই সঠিক পথে আছে</p>
              <p className="text-xs text-slate-400 leading-relaxed">
                stancl/tenancy + Kafka tenant isolation + ClickHouse per-tenant database + Redis namespaced keys —
                এই চার স্তরের isolation একসাথে করলেই তৈরি হয় enterprise-grade tracking infrastructure।
                এই architecture একবার ঠিকভাবে সেটআপ করলে ১০ ক্লায়েন্ট বা ১০,০০০ ক্লায়েন্ট —
                <strong className="text-white"> কোনো code change ছাড়াই scale করে।</strong>
              </p>
            </div>
          </Section>

          {/* FAQ */}
          <Section id="faq" icon={Info} title="সাধারণ প্রশ্ন ও উত্তর"
          ...
          {/* Summary */}
          <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-purple-900/30 to-blue-900/30 p-8 text-center">
            <div className="text-4xl mb-4">🏆</div>
            <h3 className="text-xl font-black text-white mb-3">এককথায় সারমর্ম</h3>
            <div className="grid md:grid-cols-2 gap-4 text-left mt-5 mb-5">
              <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
                <p className="text-xs font-bold text-red-400 mb-2">❌ Single DB (সব ইউজার একসাথে)</p>
                <p className="text-xs text-slate-400 leading-relaxed">ছোট প্রজেক্ট, ব্লগ বা সিম্পল টুলের জন্য ঠিক আছে। কিন্তু tracking system-এর ক্ষেত্রে মাসখানেকের মধ্যেই scalability ভেঙে পড়বে। GDPR risk বিশাল। ডাটা export প্রায় অসম্ভব।</p>
              </div>
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
                <p className="text-xs font-bold text-emerald-400 mb-2">✅ Multi-DB (Tenant-wise)</p>
                <p className="text-xs text-slate-400 leading-relaxed">Stape, Vercel, Shopify-এর মতো বড় Enterprise SaaS-এর standard pattern। Setup করতে একটু সময় লাগে, কিন্তু সিস্টেম ১০ বছর চললেও slow হবে না এবং data leak-এর কোনো chance নেই।</p>
              </div>
            </div>
            <p className="text-sm text-slate-400 leading-relaxed max-w-2xl mx-auto">
              PixelMaster-এ এই আর্কিটেকচার সম্পূর্ণরূপে implemented —
              <strong className="text-white"> MySQL (stancl/tenancy) + ClickHouse (tracking_{"{tenant_id}"}) + Redis (namespaced keys)</strong>।
